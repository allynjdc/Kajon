...
@section('content') 
			...
			<div class="reminder-container" id="reminder-container">
				...
				<div class="reminder">
					<div>
						...
						<p class="from">Notice from <img src="{{asset('images/sample.jpg')}}"> {{$reminder->user->name}} due on <span class="date">{{date('m-d-Y', strtotime($reminder->due_date))}}</span></p>
						<div class="reminder-options">
							...
							<button type="button" class="waves-effect waves-light btn grey" data-reminder-id="{{ $reminder->id }}"><i class="material-icons">block</i></button>
				...
				<div class="reminder">
					<div>
						...
						<p class="from">Notice from <img src="{{asset('images/sample.jpg')}}"> {{$reminder->user->name}} due on {{date('m-d-Y', strtotime($reminder->due_date))}}</p>
					</div>
		...
		<div class="modal-content">
			...
			<form action="{{ route('reminders.store') }}" method="POST" role='form'>
				...
				<div class="input-field">
					<input id="reminder_title" type="text" class="materialize-textarea" name="reminder_title" required="required">
					<label for="reminder_title">Notice Title</label>
				</div>
				<textarea placeholder="Reminder description" name="reminder_description" required="required"></textarea>
	...
	<div class="modal" id="edit-reminder">
		<div class="modal-content">
			...
			<form action="" method="" id="edit_reminder_form" enctype="multipart/form-data">
				...
				{{method_field('PUT')}}
				<div class="input-field">
					<input type="text" name="edit_reminder_title" id="edit_reminder_title" required="required" />
					<label for="edit_reminder_title">Notice Title</label>
				</div>
				<div class="input-field">
					<textarea class="materialize-textarea" name="edit_reminder_description" id="edit_reminder_description" required="required"></textarea>
					<label for="edit_reminder_description">Notice Description</label>
				</div>
				<div class="input-field">
					<input type="datetime-local" name="edit_due_date" id="edit_due_date" required="required" />
					<label for="edit_due_date">Due Date</label>
				</div>
				<button type="submit" class="btn waves-effect waves-light light-blue"><i class="material-icons left">edit</i> Edit Notice</button>
			</form>
		</div>
	</div>
...
@push('scripts')
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			min_due.setDate(min_due.getDate()+6);
                	...
                	$.each(data[1], function(key, value){
                		reminders += '<div class="reminder">'+
										'<div>'+
											...
											'@endif'+
										'</div>'+
										'<p class="creation">'+value["created_at"]["date"]+'</p>'+
									'</div>'
                	});
